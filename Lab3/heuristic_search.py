...
# -*- coding: utf-8 -*-
"""
My Analysis of this implementation:
code style: 3/10
easy to read: 2/10
efficiently written: 2/10
code is trash and no plesant to look at for example:
everything is very confusing, it's as if the person writing this
has no self respect.
...
class Vector:
    ...
    def __l2_min_coins_penalty__(self, uncollected, all_coins, coins_collected, max_dist):
        if (len(uncollected) <= 0):
            return 0
        return self.__l2_sq_min__(uncollected) + ((len(all_coins) - len(coins_collected)) * max_dist)
...
class SearchNode(object):
    ...
    def extract_plan(self):
        """
        Back propagate each state, return a list of states.
        remember each state is a position and grid?
        """
        plan = []
        p = self
        while p != None:
    ...
    def __lt__(self, other):
        # print(self.state.coins_collected)
        # print(other.state.coins_collected)
...
class ManhattanMaxHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        # TODO
        coins = state.grid.get_coin_locations()
        coll_ids = list(itertools.starmap(state.grid.get_coin_id, coins))
        collected = state.coins_collected
        uncollected = []
        for i in range(len(coins)):
            if coll_ids[i] not in collected:
                uncollected.append(coins[i])

        agent_pos = Vector((state.x, state.y))
        uncollected = list(map(Vector, uncollected))

        return agent_pos.__manhattan_max__(uncollected)
...
class ManhattanSumHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        agent_pos = Vector((state.x, state.y))
        uncollected = list(map(Vector, uncollected))
        return agent_pos.__manhattan_sum__(uncollected)
...
class ManhattanOrderedSumHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        agent_pos = Vector((state.x, state.y))
        uncollected = list(map(Vector, uncollected))
        return agent_pos.__manhattan_ordered_sum__(uncollected)
...
class CallMeFancyHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        agent_pos = Vector((state.x, state.y))
        uncollected = list(map(Vector, uncollected))
        all_coins = list(map(Vector, coins))

        return agent_pos.__l2_min_coins_penalty__(uncollected, all_coins, collected,
                                                  (state.grid.width ** 2 + state.grid.height ** 2))


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument("grid", help="Path to grid file (https://www.movingai.com/benchmarks/formats.html).", nargs="?",
                   default=None)
    ...
    p.add_argument("--heuristic",
                   help="Which heuristic to use. The blind heuristic will turn A* into simple Breadth-First Search.",
                   choices=sorted([x[:-9] for x in globals() if x != "Heuristic" and x.endswith("Heuristic")]),
                   default="Blind")
    args = p.parse_args()
