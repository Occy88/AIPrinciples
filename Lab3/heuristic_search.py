...
class Vector:
    ...
    def __l2__(self, other):
        d = self.__sub__(other)
        sq = d[0] ** 2, d[1] ** 2
        return int(math.sqrt(sum(sq)))

    def __manhattan__(self, other):
        return sum(self.__sub__(other))
    ...
    def __l2_min__(self, others):
        return min(list(map(self.__l2__, others)))
...
class ManhattanMaxHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        # TODO
        coins = list(map(Vector, state.grid.get_coin_locations()))
        agent_pos = Vector((state.x, state.y))
        return agent_pos.__manhattan_max__(coins)
...
class L2Heuristic(Heuristic):
    ...
    def __init__(self, grid):
        # Call the super constructor to initialize the grid and coin_location
        # variables:
        super(L2Heuristic, self).__init__(grid)
    ...
    def __call__(self, state):
        ...
        coins = list(map(Vector, state.grid.get_coin_locations()))
        agent_pos = Vector((state.x, state.y))
        return agent_pos.__l2_min__(coins)
        raise NotImplemented("ManhattanSumHeuristic has not been implemented yet")
...
class ManhattanOrderedSumHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        coins = list(map(Vector, state.grid.get_coin_locations()))
        agent_pos = Vector((state.x, state.y))
        return agent_pos.__manhattan_ordered_sum__(coins)
